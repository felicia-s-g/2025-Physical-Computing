...
// Prints from here when the right ear is pushed
...
// Prints from here when the left ear is pushed
...
// Prints from here when both ears are pushed
...
// Prints motivational quote from here once every set interval
...
String messageToPrint = ""; // Empty variable to hold the randomly selected message

void getRightMessage() {
  printer.feed(4); // Top margin

  printer.setSize('M'); // Medium text size
  printer.boldOn(); // Bold text
  printer.justify('C'); // Justified center

  randNumber = random(0, sizeof(btnRight) / sizeof(btnRight[0]));
  messageToPrint = btnRight[randNumber]; // Selects a random index from the array
  printer.println(messageToPrint); // 
  
  // Space between
  printer.feed(2);

  // Different settings for the counter
  printer.setSize('S');
  printer.boldOff();
  printer.justify('L');
